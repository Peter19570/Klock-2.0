...
const TYPE_STYLES: Record<ToastType, string> = {
  message: "bg-card border border-border text-foreground",
...
const ToastContainer = () => {
  ...
  const containerHeight = visibleToasts.reduce(
    (acc, toast) => acc + (toast.measuredHeight ?? 63),
    0,
  );

  return (
    <div
      className="pointer-events-none fixed bottom-4 right-4 z-[9999] w-[calc(100vw-2rem)] max-w-[420px]"
        ...
        {toasts.map((toast, index) => {
          ...
          return (
            <div
              ...
              className={cn(
                "absolute bottom-0 right-0 h-fit w-full rounded-xl p-4 leading-[21px] shadow-lg",
